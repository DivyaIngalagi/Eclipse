...
public class Java8MapAndFlatMapExample 
{
    public static void main(String[] args) 
    {
        ...
        List<List<String>> namesOfInstitutes = instituteList.stream().map(Institute::getLocations).collect(Collectors.toList());
         
        System.out.println(namesOfInstitutes);
         
        //Java 8 FlatMap() : Get unique locations of all institutes
         
      List<Character> locationsOfInstitutes = instituteList.stream().
        		flatMap(institute -> institute.getLocations().stream().map(s  -> s.charAt(0))).collect(Collectors.toList());
